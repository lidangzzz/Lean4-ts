...
SOLVE ALL PROBLEMS  USING MASTER AGENT AND SUBAGENT SYSTEM!!!


Here we define a goal-driven multi-agent system for solving any problem.

Goal: [[[[[make sure that all lean source code in the folder ./test_programs compiles with the typescript lean4 compiler without exception or errors and all variables are output to the terminal and the output is exactly as the original lean4 compiler output]]]]]

Criteria for success: [[[[[Run all the files with the original typescript compiler 'lean ./THE_FILE.lean' and make sure that the output is exactly as the original lean4 compiler output]]]]]
...
Goal: [[[[[Solve all remaininig problems at swe-bench lite]]]]]

Criteria for success: [[[[[make sure that every problem is solved correctly, tested and behaves as expected, design and run the tests, compile and run the test to make sure it works as expected and the problem is fully resolved]]]]]
...
Goal: [[[[[verify and fix all completed tasks and fix any issues]]]]]

Criteria for success: [[[[[double check that every problem is solved correctly, tested and behaves as expected, design and run any necessary test, compile and run the test to make sure it works as expected and the problem is fully resolved]]]]]
