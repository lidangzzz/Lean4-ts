...
def cross (ox oy : Int) (ax ay bx by : Int) : Int :=
...
def grahamScan (points : List (Int × Int)) : List (Int × Int) :=
  ...
  let rec process (remaining : List (Int × Int)) (stack : List (Int × Int)) : List (Int × Int) :=
    match remaining with
    | [] => stack
    | p :: rest =>
      match stack with
      | [] => process rest [p]
      | [_] => process rest (p :: stack)
      | top :: second :: stackRest =>
        let c := cross second.1 second.2 top.1 top.2 p.1 p.2
        if c > 0 then process rest (p :: stack)
        else process remaining (second :: stackRest)
  p0 :: process sorted []
...
def hullPerimeter (hull : List (Int × Int)) : Int :=
  let rec compute (pts : List (Int × Int)) (acc : Int) : Int :=
    ...
    | (x1, y1) :: (x2, y2) :: rest =>
      let dx := x2 - x1
      let dy := y2 - y1
      compute ((x2, y2) :: rest) (acc + dx.abs + dy.abs)
  compute hull 0

def pts1 := [(0, 0), (1, 1), (2, 2), (0, 2), (2, 0), (1, 0)]
def hull1 := grahamScan pts1
def h1c := hull1.length

def pts2 := [(0, 0), (1, 0), (2, 0), (0, 1), (1, 1), (2, 1), (0, 2), (1, 2), (2, 2)]
def hull2 := grahamScan pts2
def h2c := hull2.length

def pts3 := [(0, 0), (10, 0), (10, 10), (0, 10), (5, 5)]
